...
class TestMultiPythonInterpreter:
    ...
    class TestRestartServer:
        ...
        def test_restart_server(self, multi: MultiPythonInterpreter):
            ...
            result = multi.restart_server("restart_test")
            assert result["success"] is True
            # 重启后状态应该清空
        ...
        def test_restart_nonexistent_interpreter(self, multi: MultiPythonInterpreter):
            ...
            with pytest.raises(KeyError):
                multi.restart_server("nonexistent")

    class TestGetTools:
        """工具列表测试"""

        def test_get_tools_returns_callables(self, multi: MultiPythonInterpreter):
            """测试 get_tools 返回可调用的工具列表"""
            tools = multi.get_tools()
            assert len(tools) == 10
            for tool in tools:
                assert callable(tool)

        def test_get_tools_names(self, multi: MultiPythonInterpreter):
            """测试工具名称正确"""
            tools = multi.get_tools()
