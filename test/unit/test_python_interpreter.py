"""
PythonExecutor 和 MultiPythonExecutor 的测试套件
"""
...
from tool.python_interpreter import PythonExecutor, ExecutionResult
from tool.multi_python_interpreter import MultiPythonExecutor


class TestPythonExecutor:
    """PythonExecutor 单解释器测试"""

    @pytest.fixture
    def executor(self) -> Generator[PythonExecutor, None, None]:
        """创建临时解释器，测试后自动清理"""
        exe = PythonExecutor()
        yield exe
        exe.close()
    ...
    class TestBasicExecution:
        ...
        def test_execute_simple_expression(self, executor: PythonExecutor):
            """测试执行简单表达式"""
            result = executor.execute("1 + 1")
            assert result["success"] is True
            assert "2" in result["output"]

        def test_execute_multiple_statements(self, executor: PythonExecutor):
            """测试执行多行代码"""
            code = """
x = 10
y = 20
...
"""
        ...
        def test_execute_with_variables(self, executor: PythonExecutor):
            """测试变量状态保持"""
        ...
        def test_execute_empty_code(self, executor: PythonExecutor):
            """测试执行空代码"""
            result = executor.execute("")
            assert result["success"] is True
            assert result["output"] == ""

        def test_execute_syntax_error(self, executor: PythonExecutor):
            """测试语法错误处理"""
            result = executor.execute("if x")
            assert result["success"] is False
            assert "SyntaxError" in result["error"] or "error" in result["error"].lower()

        def test_execute_runtime_error(self, executor: PythonExecutor):
            """测试运行时错误处理"""
            result = executor.execute("1/0")
            assert result["success"] is False
            assert "ZeroDivisionError" in result["error"]

        def test_execute_timeout(self, executor: PythonExecutor):
            """测试超时功能"""
    ...
    class TestStatePersistence:
        ...
        def test_imports_persist(self, executor: PythonExecutor):
            """测试导入在多次执行间保持"""
        ...
        def test_functions_persist(self, executor: PythonExecutor):
            """测试函数定义在多次执行间保持"""
            executor.execute("""
def add(a, b):
    return a + b
""")
            result = executor.execute("print(add(3, 4))")
            assert result["success"] is True
            assert "7" in result["output"]

        def test_classes_persist(self, executor: PythonExecutor):
            """测试类定义在多次执行间保持"""
            executor.execute("""
class Person:
...
""")
            executor.execute("p = Person('Alice')")
            result = executor.execute("print(p.greet())")
            assert result["success"] is True
    ...
    class TestRestart:
        ...
        def test_restart_clears_state(self, executor: PythonExecutor):
            """测试重启后状态被清空"""
        ...
        def test_restart_server_method(self, executor: PythonExecutor):
            """测试 restart_server 方法"""
            executor.execute("y = 100")
            result = executor.restart_server()
            assert result["success"] is True
            result2 = executor.execute("print(y)")
            assert result2["success"] is False

        def test_is_alive(self, executor: PythonExecutor):
            """测试 is_alive 方法"""
    ...
    class TestScriptManagement:
        ...
        def test_save_script(self, executor: PythonExecutor):
            """测试保存脚本"""
        ...
        def test_save_script_with_description(self, executor: PythonExecutor):
            """测试保存带描述的脚本"""
        ...
        def test_execute_script(self, executor: PythonExecutor):
            """测试执行脚本"""
        ...
        def test_delete_script(self, executor: PythonExecutor):
            """测试删除脚本"""
        ...
        def test_read_script(self, executor: PythonExecutor):
            """测试读取脚本"""
        ...
        def test_read_script_not_found(self, executor: PythonExecutor):
            """测试读取不存在的脚本"""
            with pytest.raises(FileNotFoundError):
                executor.read_script("nonexistent")

        def test_list_scripts_empty(self, executor: PythonExecutor):
            """测试空脚本列表"""
            scripts = executor.list_scripts()
            assert scripts == []

        def test_list_scripts_multiple(self, executor: PythonExecutor):
            """测试列出多个脚本"""
        ...
        def test_execute_nonexistent_script(self, executor: PythonExecutor):
            """测试执行不存在的脚本"""
            with pytest.raises(FileNotFoundError):
                executor.execute_script("nonexistent")

        def test_delete_nonexistent_script(self, executor: PythonExecutor):
            """测试删除不存在的脚本"""
            with pytest.raises(FileNotFoundError):
                executor.delete_script("nonexistent")

        def test_script_auto_adds_py_extension(self, executor: PythonExecutor):
            """测试自动添加 .py 后缀"""
    ...
    class TestDependencyInstallation:
        ...
        def test_install_single_package(self, executor: PythonExecutor):
            """测试安装单个包"""
            result = executor.install_dependency("requests")
            assert result["success"] is True
            # 验证包可用
            test_result = executor.execute("import requests; print(requests.__name__)")
            assert test_result["success"] is True

        @pytest.mark.skipif(shutil.which("uv") is None, reason="uv not installed")
        def test_install_multiple_packages(self, executor: PythonExecutor):
            """测试安装多个包"""
            result = executor.install_dependency(["requests", "urllib3"])
            assert result["success"] is True

        @pytest.mark.skipif(shutil.which("uv") is None, reason="uv not installed")
        def test_install_already_installed_package(self, executor: PythonExecutor):
            """测试安装已存在的包（不应重启）"""
        ...
        @pytest.mark.skipif(shutil.which("uv") is None, reason="uv not installed")
        def test_install_with_version_spec(self, executor: PythonExecutor):
            """测试安装指定版本的包"""
    ...
    class TestWorkDir:
        ...
        def test_custom_work_dir(self, temp_dir: str):
            """测试使用自定义工作目录"""
            executor = PythonExecutor(work_dir=temp_dir)
            try:
...
class TestMultiPythonExecutor:
    """MultiPythonExecutor 多解释器测试"""

    @pytest.fixture
    def multi(self) -> Generator[MultiPythonExecutor, None, None]:
        """创建多解释器管理器"""
        m = MultiPythonExecutor()
        yield m
        # 清理所有解释器
        for name in list(m.interpreters.keys()):
            m.remove_interpreter(name)

    class TestCreateAndRemove:
        """创建和移除解释器测试"""

        def test_create_interpreter(self, multi: MultiPythonExecutor):
            """测试创建解释器"""
        ...
        def test_create_interpreter_auto_name(self, multi: MultiPythonExecutor):
            """测试自动生成解释器名称"""
        ...
        def test_create_duplicate_name(self, multi: MultiPythonExecutor):
            """测试创建重复名称的解释器"""
            multi.create_interpreter("dup")
            result = multi.create_interpreter("dup")
            assert "already exists" in result.lower()

        def test_remove_interpreter(self, multi: MultiPythonExecutor):
            """测试移除解释器"""
            multi.create_interpreter("to_remove")
            result = multi.remove_interpreter("to_remove")
            assert "closed" in result.lower() or "removed" in result.lower()
            assert "to_remove" not in multi.interpreters

        def test_remove_nonexistent_interpreter(self, multi: MultiPythonExecutor):
            """测试移除不存在的解释器"""
            with pytest.raises(KeyError):
                multi.remove_interpreter("nonexistent")

        def test_get_interpreter_not_found(self, multi: MultiPythonExecutor):
            """测试获取不存在的解释器"""
            with pytest.raises(KeyError):
    ...
    class TestMultiExecution:
        ...
        def test_multiple_interpreters_isolated(self, multi: MultiPythonExecutor):
            """测试多个解释器之间状态隔离"""
        ...
        def test_execute_nonexistent_interpreter(self, multi: MultiPythonExecutor):
            """测试在不存在解释器上执行"""
            with pytest.raises(KeyError):
    ...
    class TestMultiScripts:
        ...
        def test_save_script_per_interpreter(self, multi: MultiPythonExecutor):
            """测试每个解释器有独立的脚本"""
            multi.create_interpreter("exe1")
            multi.create_interpreter("exe2")
        ...
        def test_list_scripts_per_interpreter(self, multi: MultiPythonExecutor):
            """测试列出各解释器的脚本"""
            multi.create_interpreter("exe1")
            multi.create_interpreter("exe2")
    ...
    class TestGetTools:
        ...
        def test_get_tools_returns_callables(self, multi: MultiPythonExecutor):
            """测试 get_tools 返回可调用的工具"""
            tools = multi.get_tools()
            assert len(tools) >= 5
            for tool in tools:
                assert callable(tool)

        def test_tool_names(self, multi: MultiPythonExecutor):
            """测试工具名称"""
            tools = multi.get_tools()
            tool_names = [t.__name__ for t in tools]
            assert "create_interpreter" in tool_names
            assert "remove_interpreter" in tool_names
